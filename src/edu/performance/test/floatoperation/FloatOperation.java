...
/**
 * This class extends FloatOperation and it executes operations with double
 * float point number. Currently this operations are: - Square root calculation
 * by Newton method - Sine calculation from Math library - Pi calculation by
 * MonteCarlo method - Linpack - Cubic solve - Degree to Radian - Radian to
 * degree
 * 
 * @author Thiago
 */
public class FloatOperation extends PerformanceTest<Double> implements PerformanceTestInterface{
	 ...
	 */
	public void testAFloatOperationSqrt(double level) {
		// double level = Float.parseDouble(args[0]);
		...
		// achieved
		while (Math.abs(t - level / t) > epsilon * t) {
	...
	/**
	 * This method use the library Math to calculate sine of degree indicated by
	 * level.
	 * 
	 * @param level
	...
	/**
	 * This method use MonteCarlo methodology to calculate a approximation.
	 * 
	 * @param level
	 *            Defines the limit to the loop which is used to calculated.
	 */
	public void testAFloatOperationMonteCarlo(double level) {

		Random R = new Random(System.currentTimeMillis());

		@SuppressWarnings("unused")
		double under_curve = 0;
		...
		// System.out.println( ((double) under_curve / level) * 4.0);
	}

	@Override
	public void execute() {

		testAFloatOperationSqrt(this.getLevel());
		testAFloatOperationSine(this.getLevel());
		testAFloatOperationMonteCarlo(this.getLevel());
		testAFloatOperationLinpack(this.getLevel());
		testJFloatOperationCubic(this.getLevel());
	...
	// ----- Linpack --------------------------------------------------------
	// This code is from http://www.jeckle.de/freeStuff/jLinpack/
	// http://www.jeckle.de/freeStuff/jLinpack/Linpack.java

	/**
	 * This method executes Linpack benchmark using the implementation posted in
	 ...
	 * test the n is determinate by level.
	 * 
	 * @param level
	 *            Define the dimension of linear equation system. Max.: 1000
	 */
	void testAFloatOperationLinpack(double level) {
		level = level <= 1000 ? level : 1000;
		// int level_int = (int)this.getLevel();
		run_benchmark((int) Math.floor(level + 0.5d));
	}
	...
	public final void run_benchmark(int level) {
		double mflops_result = 0.0;
		double residn_result = 0.0;
		double time_result = 0.0;
		double eps_result = 0.0;

		double a[][] = new double[level * 2][(level * 2) + 1];
		double b[] = new double[level * 2];
		double x[] = new double[level * 2];
		double ops, total, norma, normx;
		double resid, time;
		int n, i;
		int ipvt[] = new int[level * 2];
		 ...
		 * secs" + " Norm Res: " + residn_result + " Precision: " + eps_result);
		 */
	 ...
	 * (!rcond is too small){ for (j=0,j <p,j++) dgesl(a,lda,n,ipvt,c[j][0],0);
	 * }
	 * 
	 * linpack. this version dated 08/14/78 . cleve moler, university of new
	 * mexico, argonne national lab.
	 ...
	 * blas daxpy,ddot
	 */
	final void dgesl(final double a[][], final int n, final int ipvt[],
			double b[], final int job) {
		double t;
		int k, kb, l, nm1, kp1;
	 ...
	 */
	final void daxpy(final int n, final double da, final double dx[],
			final int dx_off, final int incx, double dy[], final int dy_off,
			final int incy) {
		int i, ix, iy;
	 ...
	 */
	final double ddot(final int n, final double dx[], final int dx_off,
			final int incx, final double dy[], final int dy_off, final int incy) {
		double dtemp;
		int i, ix, iy;
	...
	/*
	 * scales a vector by a constant. jack dongarra, linpack, 3/11/78. mcj:
	 * declared some parameters as final
	 */
	final void dscal(final int n, final double da, double dx[],
			final int dx_off, final int incx) {
		int i, nincx;
	 ...
	 * linpack, 3/11/78.
	 */
	final int idamax(final int n, final double dx[], final int dx_off,
			final int incx) {
		double dmax, dtemp;
		int i, ix, itemp = 0;
	 ...
	 */
	final void dmxpy(final int n1, double y[], final int n2, final double x[],
			final double m[][]) {
		int j, i;
	...
	/**
	 * This method solves a cubic polynomial.
	 * 
	 * @param level
	 *            It is used to generate the expression coefficients.
	 */
	private void testJFloatOperationCubic(double level) {
		...
		if (R2_Q3 <= 0) {
			...
			x[1] = -2.0 * Math.sqrt(Q)
					* Math.cos((theta + 2.0 * Math.PI) / 3.0) - a1 / 3.0;
			x[2] = -2.0 * Math.sqrt(Q)
					* Math.cos((theta + 4.0 * Math.PI) / 3.0) - a1 / 3.0;
		} else {
			x = new double[1];
	...
	/**
	 * This method converts a radian "level" to degree angle .
	 * 
	 * @param level
	 *            It is the radian measure which will be converted.
	 * @return The angle in degrees from radian measure.
	 */
	double testJFloatOperationrad2deg(double rad) {
		return (180.0 * rad / (Math.PI));
	}

	/**
	 * This method converts a degree angle "level" to radians.
	 * 
	 * @param level
	 *            It is the angle in degrees which will be converted.
	 * @return The radian measure of angle.
	 */
	double testJFloatOperationdeg2rad(double deg) {
