        ...
        public static boolean assertBinaries(Context ctx, boolean showErrors) {
                ...
                try {
                        ...
                        if ((!file.exists()) && isARMv6()) {
                                //TODO copyRawFile(ctx, R.raw.iptables_g1, file, "755");
                                changed = true;
                        }
                        // Check iptables_n1
                        file = new File(ctx.getCacheDir(), "iptables_n1");
                        if ((!file.exists()) && (!isARMv6())) {
                                //TODO copyRawFile(ctx, R.raw.iptables_n1, file, "755");
                                changed = true;
                        }
                        // Check busybox
                        file = new File(ctx.getCacheDir(), "busybox_g1");
                        if (!file.exists()) {
                                //TODO copyRawFile(ctx, R.raw.busybox_g1, file, "755");
                                changed = true;
                        }
                        if (changed) {
                                //TODO Toast.makeText(ctx, R.string.toast_bin_installed, Toast.LENGTH_LONG).show();
                        }
                } catch (Exception e) {
